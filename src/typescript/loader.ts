...
export function loadBuildFile(directory: string, fileName: string, tsConfig?: string): any {
  ...
  const outputFiles = new VirtualFileList()
  ...
  const result = program.emit(undefined, (fileName, contents, bom, oe, sources) => {
    ...
    if (extname(fileName) === '.js') {
      sources?.forEach(({ fileName }) => originalPath = files.get(fileName).absolutePath)
    }

    // Add the result of the compilation to our output files
    outputFiles.add(fileName, { contents, sourceMap: true, originalPath })
  })
