...
// have inline source maps) before passing it on to the real compiler...
...
const contentMap = new Map<FilePath, string>()
  ...
  if ((typeof request === 'string') && (parent?.filename)) {
    ...
    if (isAbsolute(request) || request.startsWith(`.${sep}`) || request.startsWith(`..${sep}`)) {
      ...
      const files: FilePath[] = [
        resolve(dirname(parent.filename), request) as FilePath,
        resolve(dirname(parent.filename), request + '.ts') as FilePath,
        resolve(dirname(parent.filename), request + '/index.ts') as FilePath,
        resolve(dirname(parent.filename), request + '.js') as FilePath,
        resolve(dirname(parent.filename), request + '/index.js') as FilePath,
      ]
...
/**
 * Setup the Node module loader so that it can `require(...)` files from the
 * given `Map`.
 *
 * Files should have inline source maps, so that `source-map-support` can
 * properly cache them and resolve stack traces correctly when throwing errors.
 */
export function setupLoader(entries?: Map<FilePath, string>): void {
