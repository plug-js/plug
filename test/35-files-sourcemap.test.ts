...
import { extractSourceMap, FileSourceMap, writeSourceMap } from '../src/files/sourcemap'
...
describe('File Source Maps', () => {
  ...
  it('should produce a simple source map', async () => {
    ...
    expect(await sm?.produceSourceMap('/src/bar.txt' as FilePath, {
      combineSourceMaps: false,
      attachSources: true,
      sourceRoot: 'root/',
    })).to.eql({
      version: 3,
  ...
  it('should extract source maps from a file contents', () => {
    const { files } = mock('/foo')
    const source = files.add('source.txt', { contents: '!foobar!' })

    const path = '/foo/generated.txt' as FilePath

    expect(extractSourceMap(path, files, '// barfoo'))
        .to.eql({
          contents: '// barfoo',
          sourceMap: undefined,
    ...
    expect(extractSourceMap(path, files, `//# ${SOURCE_MAPPING_URL} = generated.txt.map \n// barfoo`))
        .to.eql({
          contents: '\n// barfoo',
          sourceMap: undefined,
          sourceMapFile: '/foo/generated.txt.map',
        })

    const inline = Buffer.from(JSON.stringify({
      version: 3,
      sources: [ 'source.txt' ],
    })).toString('base64')

    const result = extractSourceMap(path, files, `//# ${SOURCE_MAPPING_URL} = data:application/json;base64,${inline} \n// barfoo`)
    expect(result.contents).to.equal('\n// barfoo')
    expect(result.sourceMapFile).to.be.undefined
    expect(result.sourceMap).to.be.instanceOf(FileSourceMap)
    // check sources attachment
    expect(result.sourceMap?.attachedSources[0]).to.equal(source)
  })

  it('should write source maps from a file', async () => {
    const { files } = mock('/foo')
    const plain = files.add('source1.txt', { contents: 'hello, plain!', sourceMap: false })
    const withsm = files.add('source2.txt', { contents: 'hello, sourcemap!', sourceMap: { version: 3 } as any })
    const to = '/bar/out.txt' as FilePath

    expect(await writeSourceMap(to, plain, { sourceMaps: 'none' }))
        .to.eql([ [ to, 'hello, plain!' ] ])
    expect(await writeSourceMap(to, withsm, { sourceMaps: 'none' }))
        .to.eql([ [ to, 'hello, sourcemap!' ] ])

    const sm = '{"version":3,"file":"out.txt","sources":[],"names":[],"mappings":""}'

    expect(await writeSourceMap(to, plain, { sourceMaps: 'external' }))
        .to.eql([ [ to, 'hello, plain!' ] ])
    expect(await writeSourceMap(to, withsm, { sourceMaps: 'external' }))
        .to.eql([
          [ to, `hello, sourcemap!\n//# ${SOURCE_MAPPING_URL}=out.txt.map` ],
          [ to + '.map', sm ],
        ])

    const b64 = Buffer.from(sm).toString('base64')

    expect(await writeSourceMap(to, plain, { sourceMaps: 'inline' }))
        .to.eql([ [ to, 'hello, plain!' ] ])
    expect(await writeSourceMap(to, withsm, { sourceMaps: 'inline' }))
        .to.eql([ [ to, `hello, sourcemap!\n//# ${SOURCE_MAPPING_URL}=data:application/json;base64,${b64}` ] ])
  })
})
